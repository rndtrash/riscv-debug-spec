\chapter{Модуль отладки, не-ISA} \label{dm}

\begin{steps}{Модуль отладки реализует интерфейс перевода между абстрактными отладочными
    операциями и их конкретными реализациями. Он может поддерживать следующие
    операции:}
\item Дать отладчику необходимую информацию о реализации.
\item Позволить любому индивидуальному hart-у приостановить или возобновить работу.
\item Предоставить список приостановленных hart-ов.
\item Предоставить абстрактный доступ к чтению и записи в GPR-ы приостановленного hart-а.
\item Предоставить доступ к сигналу сброса, позволяющий начать отладку с самой
    первой инструкции после сброса.
\item Предоставить механизм для разрешения отладки hart-ов сразу же после сброса
      (независимо от причины сброса). (Опционально)
\item Предоставить абстрактный доступ к не-GPR регистрам \texttt{(FIXME: тавтология?)} hart-а. (Опционально)
\item Предоставить программный буфер для принуждения hart-а к выполнению произвольных инструкций. (Опционально)
\item Позволить приостановить, возобновить, и/или сбросить несколько hart-ов одновременно. (Опционально)
\item Разрешить доступ к памяти с точки зрения hart-а. (Опционально)
\item Разрешить прямой доступ к системной шине. (Опционально)
\item Группирование hart-ов. Когда хотя бы один из hart-ов приостанавливается, все остальные также останавливаются. (Опционально)
\item Ответить на внешние триггеры, останавливая каждый hart в настроенной группе. (Опционально)
\item Отправить сигнал внешнему триггеру, когда hart в группе приостанавливается. (Опционально)
\end{steps}

\begin{steps}{Чтобы соответствовать этой спецификации,
    реализация должна:}
\item Реализовать все операции из списка выше без пометки ``(Опционально)''.
\item Реализовать хотя бы один из командных механизмов программного буфера, доступа к системной шине или памяти
    абстрактного доступа.
\item
    \begin{steps}{Реализовать хотя бы одно из перечисленного:}
        \item Программный буфер.
        \item Абстрактный доступ ко всем регистрам, видимым
            ПО, работающим на hart-е, в том числе все регистры, имеющиеся
            на hart-е и перечисленным в Таблице~\ref{tab:regno}.
        \item Абстрактный доступ ко всем GPR-ам, \RcsrDcsr и
            \RcsrDpc, а также объявите, что ваша реализация следует спецификации
            ``Минимальная спецификация отладчика RISC-V \versionnum'' вместо
            ``Спецификация отладчика RISC-V \versionnum''.
    \end{steps}
\end{steps}

Один модуль отладки может отлаживать до $2^{20}$ hart-ов.

\section{Интерфейс модуля отладки} \label{dmi}

Модули отладки являются slave по отношению к шине под названием интерфейс модуля отладки (DMI). Master
этой шины -- DTM.
DMI может быть тривиальной шиной с одним master и одним slave (см. \ref{dmi_signals})
или использовать более полноценную шину, например TileLink или AMBA Advanced Peripheral
Bus. Подробности оставим проектировщикам системы.

DMI использует от 7 до 32 битов адреса.  Он поддерживает операции чтения и записи.
Начало адресного пространства используется для первого (и зачастую единственного)
модуля отладки. Дополнительное пространство можно использовать для произвольных
устройств отладки, других ядер, дополнительных модулей отладки и т.д.
Если этот DMI имеет дополнительные модули отладки, то базовый адрес следующего
модуля в адресном пространстве DMI записан в \RdmNextdm.
